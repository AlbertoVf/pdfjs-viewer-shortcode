...
function pdfjs_generator($incoming_from_handler) {
	...
	$fullscreen_link = '';
	if($fullscreen == 'true') $fullscreen_link = '<a class="pdfjs-link" href="'.$final_url.'">Pantalla completa</a><br>';
	$iframe_code = '<iframe class="pdfjs-frame" width="'.$viewer_width.'" height="'.$viewer_height.'" src="'.$final_url.'"></iframe> ';
...
function pdfjs_media_button() {
	echo '<a href="#" id="insert-pdfjs" class="button">Ver PDF</a>';
}
